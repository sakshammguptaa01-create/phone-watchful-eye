...
export function filterPhones<T extends Prediction>(preds: T[], opts: FilterOptions): T[] {
  const { threshold, strict, frameWidth, frameHeight } = opts;
  ...
  return preds.filter((p) => {
    ...
    if (ratio < 1.45 || ratio > 4) return false;

    // 3. Size gate — ignore specks that are too small to judge reliably.
    if ((w * h) / frameArea < 0.004) return false;

    // 4. Confidence floor in strict mode.
    if (p.score < Math.max(threshold, 0.6)) return false;
